...
Forward::Guides::Routes::Customization - Customizing restful resources
...
The C<-namespace> option has no effect on the URL pattern.
    ...
    # that way, a lot of flexibility is kept:
...
=head1 as
    ...
    The first part of the URL pattern can be changed with the help of the
    C<-as> option.
...
The resource id placeholder can get a custom constraint using the
C<-constraint> option:
